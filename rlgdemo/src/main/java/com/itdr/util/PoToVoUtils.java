...
public class PoToVoUtils {
    ...
    public static ProductVO ProductToProductVO(Product p) throws IOException {
        ...
        pvo.setImageHost(PropertiesUtils.getValue("imageHost"));
        pvo.setId(p.getId());
        pvo.setId(p.getId());
    ...
    public static CartProductVo getOne(Cart cart, Product p){
        ...
        if(cart.getQuantity() <= p.getStock()){
            ...
            count = p.getStock();
            cartProductVO.setLimitQuantity("库存足");
        }
    ...
    public static OrderItemVo orderItemToOderItemVo(OrderItem orderItem){
    ...
    /*获取一个BizContent对象*/
    public static BizContent getBizContent(Orders order, List<OrderItem> orderItems){
